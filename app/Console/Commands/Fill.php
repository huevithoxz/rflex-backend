...
class Fill extends Command
{
    ...
    /**
     * Execute the console command to fill bd.
     */
    public function handle()
    {
        $year = '2023';
        try {
             // Validate if the year 2023 is in the database
            ...
            $serie = $response->serie;
              ...
              if (!$yearExists) {
                foreach ($serie as $s) {
                $fecha = $s->fecha;
                $formatedDate = Carbon::parse($fecha)->format('Y-m-d');
                    $wallet = new Wallet();
        ...
        dd('Se lleno',$year,'Con exito');
        }
        dd('Ya se encuentra en la base de datos');
            return $serie;
        } catch (Exception $e) {
